...
- 普通开发改动（AI 执行规则）：在一个完整需求或 Bug 修复的**所有代码编写完成之后**，AI 必须主动运行一次 `pnpm build` 进行全局验证和打包。
- 验证失败处理：如果 `pnpm build` 报错，AI 必须优先自行修复错误，直到打包完全通过，再向用户汇报任务完成。
- 发布版本（AI 执行规则）：当用户要求“发布”、“打压缩包”时，AI 必须运行 `pnpm run zip` 命令，在 `release/` 目录生成对应的 zip 文件。
- 仅文档/规则：不构建、不验证。
