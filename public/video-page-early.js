...
// 只写最小播放器壳，阻止 115 原生 "undefined action!" 渲染
// 播放器模块由 manifest content_scripts 的 video-page.ts 顶层静态 import 加载
console.warn('[115m][DIAG] video-page-early.js loaded, top===window:', window.top===window, 'pathname:', window.location.pathname);
if(window.top===window&&/\/web\/lixian\/master\/video\//.test(window.location.pathname)&&!window.location.search.includes('m115_transcode_fallback=1')){
  console.warn('[115m][DIAG] video-page-early.js: executing document.write');
  window.__115m_early_injected = true;
  document.open();
  ...
  document.close();
  // 通知 content script：DOM 已就绪（解决 content script 先于 early script 执行的时序问题）
  window.dispatchEvent(new Event('__115m_early_ready'));
}
